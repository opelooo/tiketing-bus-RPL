...
class Jadwal extends CI_Controller {
	...
	public function tambahjadwal(){
		...
		if ($this->form_validation->run() ==  FALSE) {
			...
			$tujuan = $this->db->query("SELECT * FROM tbl_tujuan WHERE kd_tujuan ='".$this->input->post('tujuan')."'")->row_array();
	...
	public function viewjadwal($id=''){
		$data['title'] = "List Tujuan";
	 	...
	 	if ($sqlcek) {
	 		$data['asal'] = $this->db->query("SELECT * FROM tbl_tujuan WHERE kd_tujuan = '".$sqlcek['kd_tujuan']."'")->row_array();
	 		$data['jadwal'] = $sqlcek;
